module "imaging-routetable" {
  source                        = "app.terraform.io/csiseg/subnetroutetable/csi"
  version                       = "1.0.2"
  location                      = var.loc
  disable_bgp_route_propagation = "true"
  teamname                      = "routetable-imaging-ebg-${var.env}-${var.loc}"
...
module "routetable-route" {
  ...
  providers = {
    ...
    azurerm.src  = azurerm.digital-non-prod
  }
  route_table_name            = module.imaging-routetable.name
  resource_group_name         = "rg-coreservices-ebg-nonprod-${var.loc}"
  routename                   = "imaging-${var.env}-spoke-routing-to-hub-vnet"
