module "ddacloudcore-routetable" {
  source                        = "app.terraform.io/csiseg/subnetroutetable/csi"
  version                       = "1.0.2"
  location                      = var.loc
  disable_bgp_route_propagation = "true"
  teamname                      = "routetable-ddacloudcore-${var.env}-${var.loc}"
...
module "routetable-route" {
  ...
  route_table_name            = module.ddacloudcore-routetable.name
  resource_group_name         = "rg-coreservices-ebg-nonprod-${var.loc}"
  routename                   = "ddacloudcore-${var.env}-spoke-routing-to-hub-vnet"
