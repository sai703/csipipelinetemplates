module "statementscloudcore-routetable" {
  source                        = "app.terraform.io/csiseg/subnetroutetable/csi"
  version                       = "1.0.2"
  location                      = var.loc
  disable_bgp_route_propagation = "true"
  teamname                      = "routetable-statementscloudcore-${var.env}-${var.loc}"
...
module "routetable-route" {
  ...
  route_table_name            = module.statementscloudcore-routetable.name
  resource_group_name         = "rg-coreservices-ebg-nonprod-${var.loc}"
  routename                   = "statementscloudcore-${var.env}-spoke-routing-to-hub-vnet"
