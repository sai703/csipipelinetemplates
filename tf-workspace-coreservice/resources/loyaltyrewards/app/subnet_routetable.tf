module "loyalty-routetable" {
  source                        = "app.terraform.io/csiseg/subnetroutetable/csi"
  version                       = "1.0.2"
  location                      = var.loc
  disable_bgp_route_propagation = "true"
  teamname                      = "routetable-loyaltyrewards-ebg-${var.env}-${var.loc}"
...
module "routetable-route" {
  ...
  route_table_name            = module.loyalty-routetable.name
  resource_group_name         = "rg-coreservices-ebg-${var.route_env}-${var.loc}"
  routename                   = "loyaltyrewards-${var.env}-spoke-routing-to-hub-vnet"
